...
@DataJpaTest
class SaksdataRepositoryTest: PostgresIntegrationTestBase() {
    @Autowired
    ...
    @Test
    fun `findByAvsluttetAvSaksbehandlerBetweenV2 with orphaned kvalitetsvurdering reference throws NPE`() {
        // This test reproduces the production error:
        // java.lang.NullPointerException: null cannot be cast to non-null type KvalitetsvurderingV2
        // This happens when a Saksdata record has a kvalitetsvurderingReference pointing to a
        // KvalitetsvurderingV2 that doesn't exist (orphaned reference)

        val utfoerendeSaksbehandler = "abc123"

        // Create a Saksdata with a kvalitetsvurderingReference pointing to a non-existent KvalitetsvurderingV2
        val orphanedId = UUID.randomUUID()
        val saksdataWithOrphanedReference = Saksdata(
            utfoerendeSaksbehandler = utfoerendeSaksbehandler,
            tilknyttetEnhet = "4295",
            kvalitetsvurderingReference = KvalitetsvurderingReference(
                id = orphanedId, // This ID doesn't exist in kvalitetsvurdering_v2 table
                version = 2,
            ),
            created = LocalDateTime.of(LocalDate.of(2022, Month.JANUARY, 1), LocalTime.NOON),
            avsluttetAvSaksbehandler = LocalDateTime.of(LocalDate.of(2022, Month.JANUARY, 12), LocalTime.NOON),
        ...
        // Also create a valid Saksdata with an existing KvalitetsvurderingV2
        val saksdataWithValidReference = Saksdata(
            utfoerendeSaksbehandler = utfoerendeSaksbehandler,
            tilknyttetEnhet = "4295",
            kvalitetsvurderingReference = KvalitetsvurderingReference(
                id = kvalitetsvurderingV2Repository.save(KvalitetsvurderingV2()).id,
        ...
        saksdataRepository.saveAll(
            mutableListOf(
                saksdataWithOrphanedReference,
                saksdataWithValidReference,
            )
        )
        ...
        // This should throw NullPointerException because of the LEFT JOIN returning null
        // for the orphaned kvalitetsvurdering reference
        val finished =
            saksdataRepository.findByAvsluttetAvSaksbehandlerBetweenV2(
                fromDateTime = LocalDateTime.of(LocalDate.of(2022, Month.JANUARY, 1), LocalTime.MIN),
                toDateTime = LocalDateTime.of(LocalDate.of(2022, Month.JANUARY, 15), LocalTime.MIN),
        ...
        // After the fix, this should only return the Saksdata with a valid KvalitetsvurderingV2
        // or handle the orphaned reference gracefully
